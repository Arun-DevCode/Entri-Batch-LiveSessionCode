...
const server = http.createServer((request, response) => {
  ...
  if (request.url === "/register" && request.method === "POST") {
    response.end(JSON.stringify({ message: "User registered success!" }));
  }

  if (request.url === "/remove-account" && request.method === "DELETE") {
    response.end(JSON.stringify({ message: "Your account has been removed!" }));
  }

  if (request.url === "/login" && request.method === "POST") {
    response.end(JSON.stringify({ Message: "User loggedin success.." }));
  } else {
    response.statusCode = 400;
    response.end(JSON.stringify({ message: "Method were wrong.." }));
  }
