...
function RegisterPage() {
  ...
  async function handleUserRegister(event) {
    ...
    // Field validationhttps://documenter.getpostman.com/view/47792807/2sB3WvKxHU
    if (!formData.name || !formData.email || !formData.password) {
      return alert("Fill all fields.");
    }
    try {
      const response = await fetch("http://localhost:8142/register", {
        method: "POST",
        headers: {
        ...
        <div className="shadow p-4 rounded-lg border border-gray-200">
          ...
          <form className="flex flex-col">
            ...
            <span>
              ...
              <Link to={"/"} className="text-emerald-500 cursor-pointer">
                Login here
              </Link>
            </span>
          </form>
